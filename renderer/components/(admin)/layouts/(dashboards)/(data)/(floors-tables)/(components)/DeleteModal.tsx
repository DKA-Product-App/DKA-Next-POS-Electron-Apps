...
export const DeleteModal = ({ id, name, onDeleted }: { id: string; name?: string; onDeleted?: () => void }) => {
    ...
    const openConfirm = React.useCallback(() => {
        setSwalProps({
            show: true,
            icon: 'warning',
            title: 'Hapus Data Meja Ini ?',
            html: `Anda akan menghapus <b>${name || 'Meja ini'}</b>. Tindakan ini tidak dapat dibatalkan.`,
            ...
            onConfirm: () => {
                ...
                window?.api?.invoke?.('api.config.data.floors.tables:delete.one', { id })
                    .then((resolve) => {
                        console.log(resolve);
                        setSwalProps({
                            show: true,
                    ...
                    .catch((err) => {
                        ...
                        setSwalProps({
                            ...
                            theme: mode,
                            timer: 5000,
                            text: err?.msg || err?.message || 'Gagal menghapus Meja',
