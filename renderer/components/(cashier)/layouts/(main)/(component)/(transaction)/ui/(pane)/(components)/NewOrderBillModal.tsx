...
export default function NewOrderBillModal({
    ...
    const [transactionBill, setTransactionBill] =
        React.useState<TransactionBill | undefined>(undefined)
    ...
    const isDark =
        ...
        (theme.palette as any)?.colorScheme === 'dark'

    const isSplitMode = mode === 'split'
    const baseIcon = isSplitMode ? (
        <CallSplitRounded sx={{ fontSize: 36 }} />
    ) : (
        <ReceiptLongRounded sx={{ fontSize: 36 }} />
    )
    ...
    const tooltip = `Buat Tagihan (${isSplitMode ? 'Split' : 'Keseluruhan'}) — ${
        items.length
    } item`
    ...
    const buildIdemKey = (
        ...
        itemsLoaded: TransactionBatchItem[],
    ) => {
    ...
    const closeWithDialog = React.useCallback(() => {
        setSwalProps({
            ...
            didOpen: (popupEl) => {
                const container = (popupEl as HTMLElement)?.closest(
                    '.swal2-container',
                ) as HTMLElement | null
            ...
            .then(({ data }) => {
                ...
                const BillTemporary: TransactionBill = {
                    ...
                    items: data.map((it) => ({
                        ...
                        status: !godMode
                            ? snap.itemsGod.some((id) => id === it.id)
                            : true,
                    })),
                    paid: { status: false },
                    meta: { request_key }, // ⬅️ kunci idempotensi ke backend
                }
    ...
    React.useEffect(() => {
        ...
        if (!snap?.txId || (snap.items?.length ?? 0) === 0) {
            setLoading(false)
            return
        }
    ...
    return (
        <>
            ...
            >
                <DialogTitle
                    sx={{ display: 'flex', alignItems: 'center', pr: 1.5, gap: 1 }}
                >
                    <Typography variant="h6" fontWeight={800}>
                        {isSplitMode
                            ? 'Preview Tagihan (Split)'
                            : 'Preview Tagihan (Keseluruhan)'}
                    </Typography>
                    <Stack
                        direction="row"
                        spacing={0.5}
