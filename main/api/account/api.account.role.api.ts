...
export function AccountsRoles(mainWindow ?: BrowserWindow) {
    // CREATE
    mainWindow?.webContents?.ipc?.handle?.("api.account.role:create", (_event, args) => {

        const toPath = compile(`/v${ApiConfig.version}/resources/account/role`);
    ...
    // READ ALL
    mainWindow?.webContents?.ipc?.handle?.("api.account.role:read.all", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/account/role`);
    ...
    // READ ONE
    mainWindow?.webContents?.ipc?.handle?.("api.account.role:read.one", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/account/role/:id`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
                url: toPath(args),
    ...
    // UPDATE ONE
    mainWindow?.webContents?.ipc?.handle?.("api.account.role:update.one", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/account/role/:id`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
    ...
    // DELETE ONE
    mainWindow?.webContents?.ipc?.handle?.("api.account.role:delete.one", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/account/role/:id`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
    ...
    // PRINT ONE
    mainWindow?.webContents?.ipc?.handle?.("api.account.role:print", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/account/role/print`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
                url: toPath(),
