...
export function ApiConfigBaseBranch(mainWindow?: BrowserWindow) {

    // CREATE
    mainWindow?.webContents?.ipc?.handle?.("api.config.base.branch:create", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/config/base/branch`);
    ...
    // READ ALL
    mainWindow?.webContents?.ipc?.handle?.("api.config.base.branch:read.all", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/config/base/branch`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
    ...
    // READ ONE
    mainWindow?.webContents?.ipc?.handle?.("api.config.base.branch:read.one", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/config/base/branch/:id`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
                url: toPath(args),
    ...
    // UPDATE ONE
    mainWindow?.webContents?.ipc?.handle?.("api.config.base.branch:update.one", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/config/base/branch/:id`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
    ...
    // DELETE ONE
    mainWindow?.webContents?.ipc?.handle?.("api.config.base.branch:delete.one", (_event, args) => {
        const toPath = compile(`/v${ApiConfig.version}/resources/config/base/branch/:id`);
        return new Promise(async (resolve, reject) => {
            const ApiRequestInstance = await getApi();
            return ApiRequestInstance({
...
export default ApiConfigBaseBranch;
